...
let nutrients; // NEW nutrient map
...
function mutateGenes(p, m = 0.1) {
  return {
    ...
    hue: clamp(p.hue + (Math.random()*2-1)*m*24,85,150),
...
let tips=[],segments=0,grazers=[],predators=[];
...
function reset(){
  ...
  tips=[]; grazers=[]; predators=[];
  segments=0;
}

function drawSegment(x1,y1,x2,y2,w,g,graze=0){
  const hue=g.hue*(1-graze)+52*graze;
...
function step(){
  ...
  updateNutrients(); // nutrient regrowth
  ...
  for(const t of tips){
    ...
    // ENERGY RECYCLING via nutrients
    const food = nutrients[idx];
    t.energy += 0.18 * food;
    nutrients[idx] *= 0.92;

    const ageStop=(1-Math.min(1,t.energy/320))*0.028;
    const effectiveStop=(stopChance+ageStop)*g.stopBias;
    ...
    t.angle += (Math.random()*2-1)*g.jitter + wind*0.55 + g.turnBias;
    ...
    if(Math.random()<branchChance*0.88*g.branchBias && t.energy>15){
      ...
      newTips.push(new Tip(t.x,t.y,t.angle-split,t.width*0.75,t.energy*0.63,mutateGenes(g,0.08)));
      newTips.push(new Tip(t.x,t.y,t.angle+split,t.width*0.75,t.energy*0.63,mutateGenes(g,0.08)));
      t.energy*=0.72;
    }
  }
  ...
  // predator starvation stabilizer
